...
def datestamp():
    """Return a string of today's date in YYYY-MM-DD format."""
    return '{:%F}'.format(datetime.now())

def timedatestamp(sep=' '):
    """Return a string of today's time and date in 12-hour HH:MM:SS(AM|PM)
    YYYY-MM-DD format, separated by sep, which defaults to a space."""
    return sep.join([timestamp(), datestamp()])

def timestamp():
    """Return a string of the current time in 12-hour HH:MM:SS(AM|PM) format."""
    return '{:%I:%M:%S%p}'.format(datetime.now())
